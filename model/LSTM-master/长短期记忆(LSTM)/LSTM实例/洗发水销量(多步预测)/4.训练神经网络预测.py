...
def loadData(fname1):
    ...
    :return:
    '''
    data = read_csv(fname1, header=0, parse_dates=[0], index_col=0, squeeze=True)
    return data
...
# 加载数据
series = read_csv('E:/xsyc/xsyc/data/sales.csv', header=0, parse_dates=[0], index_col=0, squeeze=True)
# 参数配置
n_lag = 1       # 用一个数据
n_seq = 12       # 预测三个数据
n_test = 10    # 测试数据为10组
