...
def selectArgsForModel(D_data):
    ...
    for p in range(pmax +1):
        ...
        for q in range(qmax+1):
            try:
                value = ARIMA(D_data, (p, 2, q)).fit().bic
                temp.append(value)
            except:
                temp.append(None)
...
def bulidModel(data,p,q):
    ...
    '''
    try:
        model = ARIMA(data, (p,2,q)).fit()
    except:
        # 平稳性检测不通过，参考：https://github.com/statsmodels/statsmodels/issues/1155/
        model = ARIMA(data, (4,2,1)).fit(trend='nc')
    try:
        # 检测模型是否可用
        model.summary2()
    except:
        # 模型平衡性查，就固定p,d,q固定为4，1，1
        model = ARIMA(data, (4,2,1)).fit()
        model.summary2()
    # 保存模型
    # model.save('model.pkl')
    return model
    '''
    if(isModelOk(data,p,2,q)):
        model = ARIMA(data, (p,2,q)).fit()
        model.summary2()
    elif(isModelOk(data,p,2,q,'nc')):
        model = ARIMA(data, (p,2,q)).fit(trend='nc')
        model.summary2()
    elif(isModelOk(data,4,2,1)):
        model = ARIMA(data, (4,2,1)).fit()
        model.summary2()
        
    elif(isModelOk(data,4,2,1,'nc')):
        model = ARIMA(data, (4,2,1)).fit(trend='nc')
        model.summary2()    
        
    elif(isModelOk(data,4,1,1)):
        model = ARIMA(data, (4,1,1)).fit()
        model.summary2()
    elif(isModelOk(data,4,1,1,'nc')):
        model = ARIMA(data, (4,1,1)).fit(trend='nc')
        model.summary2()
...
def isModelOk(data,p,d,q,tr='c'):
    try:
        ARIMA(data, (p,d,q)).fit(trend=tr)
        return True
    except:
        return False
