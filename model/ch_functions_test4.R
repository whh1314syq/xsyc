library(caret)
library(forecast)
fun_arima = function(history_raw,prenum=4){
  # ##
  ...
  result <- tryCatch({
    ...
    pred <- forecast(model, h=prenum)%>%data.frame()
...
fun_stl= function(history_raw,prenum=4){
  # ##
  ...
  result <- tryCatch({
    ...
    pred  <- model%>%forecast(method="naive", h=prenum)%>%data.frame()
    list(model=model,pred=pred)
  },error=function(e){
...
fun_ets = function(history_raw,prenum=4){
  # ##
  # 数据长度短于一个周期长度时，模型中仍然可以使用frequency=12
  # Note:columns "sales" & "timestamp" cannot be renamed
...
fun_arima2 = function(history_raw,prenum=4){
  # ##
  # 数据长度短于一个周期长度时，模型中仍然可以使用frequency=12
  # Note:columns "sales" & "timestamp" cannot be renamed. 
  # Given m samples, m-12 samples will be in the 1st folds,m-6 samples in the 7th folds.
